...
**Status**: Observer framework implemented (Phase 2a - December 2025); VBI observer planned for Phase 2b
...
**Phase 2a Implementation**: The Observer framework is complete with base classes for all observers. The first concrete observer (VITSQualityObserver) has been implemented and tested. VBI Observer is planned for Phase 2b.

---

### 6.2 Observation Common Fields

**Implementation**: Base classes defined in `orc/core/include/observer.h`
...
- Confid2 Implementation**: `VITSQualityObserver` in `orc/core/include/vits_observer.h`
...
class VITSQualityObserver : public Observer {
    // Configuration:
    // PAL: White line 19 (12μs, 8μs), Black line 22 (12μs, 50μs)
    // NTSC: White line 20/13 (multiple configs), Black line 1 (10μs, 20μs)
};
```

**Processing**:
1. Extracts specific regions from VITS lines based on format (PAL/NTSC)
2. Converts 16-bit samples to IRE units
3. Validates white level is in acceptable range (90-110 IRE)
4. Calculates PSNR using 100 IRE as reference signal
5. Formula: `PSNR = 20 * log10(100 / std_deviation)`

**Output**:
- White SNR: Quality metric from white flag reference (optional)
- Black PSNR: Quality metric from black level (optional)
- Confidence: HIGH (both metrics), MEDIUM (one metric), NONE (no metrics)

**Tested**: Integration tests with real PAL and NTSC TBC files showing SNR values of 33-36 dB for both white and black measurements.

**Hint Usage**: TBC metadata VITS metrics (Section 4.4) will be used as hints in future enhancements for validation and confidence boosting
- Detection basis
...
**Phase 2b Implementation**: Planned. VBI data from TBC metadata is available via `TBCMetadataReader::read_vbi()`. See Section 4.4 for the `VBIData` structure. This will be used as hints for the Phase 2b VBI Observer to corroborate sample-derived decoding.
...
#### Phase 2a - Observer Framework and VITS (COMPLETED - December 2025)
