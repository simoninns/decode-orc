    ...
    size_t max_fields)
{
    ...
    try {
        ...
        ORC_LOG_INFO("DropoutAnalysisDecoder: Processing {} fields at node '{}'",
                    field_count, node_id);
        
        // For large datasets, populate in smaller batches to show initial results faster
        // The observation cache will handle caching, so repeated access is fast
        ...
        for (size_t i = 0; i < field_count; ++i) {
            ...
            if (i > 0 && i % BATCH_SIZE == 0) {
                ORC_LOG_INFO("DropoutAnalysisDecoder: Processed {}/{} fields", i, field_count);
            }
