...
std::vector<ParameterDescriptor> AudioSinkStage::get_parameter_descriptors(VideoSystem project_format) const {
    (void)project_format;
...
bool AudioSinkStage::trigger(
    ...
    try {
        ...
        for (FieldID fid = start_field; fid < end_field; ++fid) {
            ...
            // Update progress
            if ((fid.value() - start_field.value()) % 100 == 0) {
                double progress = static_cast<double>(fid.value() - start_field.value()) / total_fields * 100.0;
