...
{
    ORC_LOG_DEBUG("ChromaSink: render_preview_field called for field {}", field_id.value());
    
    if (!input || !input->has_field(field_id)) {
        ORC_LOG_WARN("ChromaSink preview: Invalid input for field {}", field_id.value());
        return input;
    }
    
    // Get video parameters
    auto video_params_opt = input->get_video_parameters();
    if (!video_params_opt) {
        ORC_LOG_WARN("ChromaSink preview: No video parameters available");
        return input;
    }
    const VideoParameters& videoParams = *video_params_opt;
    
    // The chroma decoder needs TWO fields to make a frame
    // We need to get both the current field and its partner
    // For field N, its partner is N+1 if N is even, or N-1 if N is odd
    FieldID field_a = field_id;
    FieldID field_b = FieldID(field_id.value() ^ 1); // Toggle last bit (0->1, 1->0, 2->3, etc.)
    
    // Make sure we process in order (lower field ID first)
    if (field_b.value() < field_a.value()) {
        std::swap(field_a, field_b);
    }
    
    // Check if we have both fields
    if (!input->has_field(field_a) || !input->has_field(field_b)) {
        ORC_LOG_WARN("ChromaSink preview: Missing field pair for field {}", field_id.value());
        return input;
    }
    
    // Convert both fields to SourceFields
    SourceField sourceField_a = convertToSourceField(input.get(), field_a);
    SourceField sourceField_b = convertToSourceField(input.get(), field_b);
    
    if (sourceField_a.data.empty() || sourceField_b.data.empty()) {
        ORC_LOG_WARN("ChromaSink preview: Failed to convert field pair {}/{}", field_a.value(), field_b.value());
        return input;
    }
    ...
    // Decode single field (we need 2 fields minimum for a frame)
    // For preview, we'll decode just this field by duplicating it
    ...
    ORC_LOG_DEBUG("ChromaSink preview: Decoded frame from fields {}/{}, size {}x{}", 
                  field_a.value(), field_b.value(), width, height);
    ...
    ORC_LOG_DEBUG("ChromaSink preview: IRE levels - black={}, white={}, range={}", 
                  blackIRE, whiteIRE, ireRange);
    
    // Create RGB data as 8-bit RGB888 (to be stored in a custom representation)
    std::vector<uint8_t> rgbData888(width * height * 3);
    ...
    for (int32_t y = 0; y < height; y++) {
        ...
        for (int32_t x = 0; x < width; x++) {
            ...
            // This matches the OutputWriter's RGB48 conversion
            ...
            // Convert Y'UV to R'G'B' using same coefficients as OutputWriter
            // [Poynton eq 28.6 p337]
            double R = rY + (1.139883 * rV);
            double G = rY + (-0.394642 * rU) + (-0.580622 * rV);
            double B = rY + (2.032062 * rU);
            
            // Clamp and convert to 8-bit
            R = std::max(0.0, std::min(65535.0, R)) / 256.0;
            G = std::max(0.0, std::min(65535.0, G)) / 256.0;
            B = std::max(0.0, std::min(65535.0, B)) / 256.0;
            
            int offset = (y * width + x) * 3;
            rgbData888[offset + 0] = static_cast<uint8_t>(R);
            rgbData888[offset + 1] = static_cast<uint8_t>(G);
            rgbData888[offset + 2] = static_cast<uint8_t>(B);
        }
    }
    
    // Pack RGB888 data into 16-bit samples for storage
    // We'll store 3 samples per pixel: R (16-bit), G (16-bit), B (16-bit)
    std::vector<uint16_t> rgbData(width * height * 3);
    for (size_t i = 0; i < rgbData888.size(); i++) {
        rgbData[i] = static_cast<uint16_t>(rgbData888[i]) << 8;  // Scale to 16-bit
    }
    
    // Create a simple in-memory VideoFieldRepresentation for RGB data
    // This represents a FULL FRAME (both fields decoded together)
    class RGBFieldRepresentation : public VideoFieldRepresentation {
    private:
        FieldID field_a_;
        FieldID field_b_;
        FieldDescriptor descriptor_;
        std::vector<uint16_t> data_;  // RGB data packed as 16-bit samples (R,G,B triplets)
        std::vector<uint8_t> rgb888_data_;  // RGB888 data for direct access
        VideoParameters video_params_;
        std::shared_ptr<const VideoFieldRepresentation> original_input_;  // Keep reference to preserve hints
        
    public:
        RGBFieldRepresentation(FieldID fa, FieldID fb, const FieldDescriptor& desc, 
                              std::vector<uint16_t> data, std::vector<uint8_t> rgb888, 
                              const VideoParameters& vp,
                              std::shared_ptr<const VideoFieldRepresentation> original)
            : VideoFieldRepresentation(
              ...
              field_a_(fa), field_b_(fb), descriptor_(desc), data_(std::move(data)), 
              rgb888_data_(std::move(rgb888)), video_params_(vp), original_input_(original) {}
        ...
        const sample_type* get_line(FieldID id, size_t line) const override {
            ...
            return &data_[line * descriptor_.width * 3];  // 3 samples per pixel (R,G,B)
        }
        ...
        // Pass through hints from the original input
        std::vector<DropoutRegion> get_dropout_hints(FieldID id) const override {
            if (original_input_ && (id == field_a_ || id == field_b_)) {
                return original_input_->get_dropout_hints(id);
            }
            return {};
        }
        ...
        std::optional<FieldParityHint> get_field_parity_hint(FieldID id) const override {
            if (original_input_ && (id == field_a_ || id == field_b_)) {
                return original_input_->get_field_parity_hint(id);
            }
            return std::nullopt;
        }
        ...
        std::optional<FieldPhaseHint> get_field_phase_hint(FieldID id) const override {
            if (original_input_ && (id == field_a_ || id == field_b_)) {
                return original_input_->get_field_phase_hint(id);
            }
            return std::nullopt;
        }
        ...
        std::vector<std::shared_ptr<Observation>> get_observations(FieldID id) const override {
            if (original_input_ && (id == field_a_ || id == field_b_)) {
                return original_input_->get_observations(id);
            }
            return {};
        }
        
        // Custom method to identify this as RGB data
        std::string type_name() const override { return "RGBFieldRepresentation"; }
        
        // Accessor for RGB888 data
        const std::vector<uint8_t>& get_rgb888_data() const { return rgb888_data_; }
    };
    
    // Create field descriptor for RGB field
    auto desc_opt = input->get_descriptor(field_id);
    if (!desc_opt) {
        return input;
    }
    ...
    ORC_LOG_DEBUG("ChromaSink preview: Returning RGB representation for fields {}/{}, size {}x{}", 
                  field_a.value(), field_b.value(), width, height);
    
    return std::make_shared<RGBFieldRepresentation>(field_a, field_b, rgbDesc, std::move(rgbData), std::move(rgbData888), videoParams, input);
}
