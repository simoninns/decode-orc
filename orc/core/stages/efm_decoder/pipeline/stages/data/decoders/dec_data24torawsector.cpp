/*
 * File:        dec_data24torawsector.cpp
 * Module:      efm-decoder-data
 * Purpose:     EFM Data24 to data decoder
 *
 * SPDX-License-Identifier: GPL-3.0-or-later
 * SPDX-FileCopyrightText: 2025-2026 Simon Inns
 */

#include "dec_data24torawsector.h"
#include <logging.h>
#include <algorithm>
#include <cstddef>
#include <cstdio>
#include <sstream>
#include <utility>

// See https://www.domesday86.com/?page_id=2678#CD_Sector_descrambling
// for the code used to generate this look-up table
static constexpr uint8_t unscrambleTable[2352] = {
    
};

Data24ToRawSector::Data24ToRawSector()
    : m_currentState(WaitingForSync),
    m_sectorDataStart(0),
    m_sectorErrorStart(0),
    m_sectorPaddedStart(0),
      m_missedSyncPatternCount(0),
      m_goodSyncPatternCount(0),
      m_badSyncPatternCount(0),
      m_validSectorCount(0),
      m_discardedBytes(0),
      m_discardedPaddingBytes(0),
      m_syncLostCount(0),
      m_syncHuntAttempts(0),
      m_syncHuntNoPatternCount(0),
      m_syncHuntFalsePositiveCount(0),
      m_syncHuntZeroPositionFalsePositiveCount(0),
      m_syncHuntFullPaddingSkipCount(0),
      m_syncHuntDiscardedBytes(0),
      m_syncHuntDiscardedPaddingBytes(0)
{}

void Data24ToRawSector::resetSyncHuntCounters()
{
    m_syncHuntAttempts = 0;
    m_syncHuntNoPatternCount = 0;
    m_syncHuntFalsePositiveCount = 0;
    m_syncHuntZeroPositionFalsePositiveCount = 0;
    m_syncHuntFullPaddingSkipCount = 0;
    m_syncHuntDiscardedBytes = 0;
    m_syncHuntDiscardedPaddingBytes = 0;
}

void Data24ToRawSector::logSyncHuntSummary(const char* outcome)
{
    if (m_syncHuntAttempts == 0) {
        return;
    }

    LOG_DEBUG("Data24ToRawSector::waitingForSync(): Sync hunt {} after {} attempts (no-pattern: {}, false-positives: {}, zero-pos false-positives: {}, full-padding skips: {}, discarded bytes: {}, discarded padding bytes: {}, buffered bytes remaining: {})",
              outcome,
              m_syncHuntAttempts,
              m_syncHuntNoPatternCount,
              m_syncHuntFalsePositiveCount,
              m_syncHuntZeroPositionFalsePositiveCount,
              m_syncHuntFullPaddingSkipCount,
              m_syncHuntDiscardedBytes,
              m_syncHuntDiscardedPaddingBytes,
              bufferedBytes());

    resetSyncHuntCounters();
}

void Data24ToRawSector::pushSection(const Data24Section &data24Section)
{
    // Add the data to the input buffer
    m_inputBuffer.push_back(data24Section);

    // Process the state machine
    processStateMachine();
}

RawSector Data24ToRawSector::popSector()
{
    // Return the first item in the output buffer
    RawSector sector = std::move(m_outputBuffer.front());
    m_outputBuffer.pop_front();
    return sector;
}

size_t Data24ToRawSector::bufferedBytes() const
{
    return m_sectorData.size() - m_sectorDataStart;
}

void Data24ToRawSector::consumeBufferedBytes(size_t count, bool consumePadded)
{
    m_sectorDataStart += count;
    m_sectorErrorStart += count;
    if (consumePadded) {
        m_sectorPaddedStart += count;
    }
    compactBuffers();
}

void Data24ToRawSector::compactBuffers()
{
    constexpr size_t compactThreshold = 64 * 1024;

    if (m_sectorDataStart == 0 && m_sectorErrorStart == 0 && m_sectorPaddedStart == 0) {
        return;
    }

    if (m_sectorDataStart >= m_sectorData.size()) {
        m_sectorData.clear();
        m_sectorDataStart = 0;
    }
    if (m_sectorErrorStart >= m_sectorErrorData.size()) {
        m_sectorErrorData.clear();
        m_sectorErrorStart = 0;
    }
    if (m_sectorPaddedStart >= m_sectorPaddedData.size()) {
        m_sectorPaddedData.clear();
        m_sectorPaddedStart = 0;
    }

    if (m_sectorDataStart >= compactThreshold && m_sectorDataStart < m_sectorData.size()) {
        m_sectorData.erase(m_sectorData.begin(), m_sectorData.begin() + static_cast<std::ptrdiff_t>(m_sectorDataStart));
        m_sectorDataStart = 0;
    }
    if (m_sectorErrorStart >= compactThreshold && m_sectorErrorStart < m_sectorErrorData.size()) {
        m_sectorErrorData.erase(m_sectorErrorData.begin(), m_sectorErrorData.begin() + static_cast<std::ptrdiff_t>(m_sectorErrorStart));
        m_sectorErrorStart = 0;
    }
    if (m_sectorPaddedStart >= compactThreshold && m_sectorPaddedStart < m_sectorPaddedData.size()) {
        m_sectorPaddedData.erase(m_sectorPaddedData.begin(), m_sectorPaddedData.begin() + static_cast<std::ptrdiff_t>(m_sectorPaddedStart));
        m_sectorPaddedStart = 0;
    }
}

bool Data24ToRawSector::isReady() const
{
    // Return true if the output buffer is not empty
    return !m_outputBuffer.empty();
}

void Data24ToRawSector::processStateMachine()
{
    while (!m_inputBuffer.empty()) {
        const Data24Section &data24Section = m_inputBuffer.front();

        // Add the data24 section's data to the sector data buffer
        m_sectorData.reserve(m_sectorData.size() + 2352);
        m_sectorErrorData.reserve(m_sectorErrorData.size() + 2352);
        m_sectorPaddedData.reserve(m_sectorPaddedData.size() + 2352);

        for (int i = 0; i < 98; i++) {
            const Data24 &frame = data24Section.frameRef(i);

            // Data
            const std::vector<uint8_t> &frameData = frame.dataRef();
            m_sectorData.insert(m_sectorData.end(), frameData.begin(), frameData.end());

            // Add the error and padding data (byte-addressable)
            const std::vector<uint8_t> &frameErrorData = frame.errorDataBytesRef();
            const std::vector<uint8_t> &framePaddedData = frame.paddedDataBytesRef();
            m_sectorErrorData.insert(m_sectorErrorData.end(), frameErrorData.begin(), frameErrorData.end());
            m_sectorPaddedData.insert(m_sectorPaddedData.end(), framePaddedData.begin(), framePaddedData.end());
        }

        m_inputBuffer.pop_front();

        switch (m_currentState) {
            case WaitingForSync:
                m_currentState = waitingForSync();
                break;
            case InSync:
                m_currentState = inSync();
                break;
            case LostSync:
                m_currentState = lostSync();
                break;
        }
    }

    if (m_currentState == WaitingForSync && m_syncHuntAttempts > 0 && bufferedBytes() < 2352) {
        logSyncHuntSummary("paused (input exhausted)");
    }
}

Data24ToRawSector::State Data24ToRawSector::waitingForSync()
{
    State nextState = WaitingForSync;

    // Is there enough data in the buffer to form a sector?
    if (bufferedBytes() < 2352) {
        // Get more data and try again
        nextState = WaitingForSync;
        return nextState;
    }

    m_syncHuntAttempts++;

    // Does the sector data contain the sync pattern? Use std::search to find it
    auto it = std::search(m_sectorData.begin() + static_cast<std::ptrdiff_t>(m_sectorDataStart), m_sectorData.end(),
                         m_syncPattern.begin(), m_syncPattern.end());
    int syncPatternPosition = (it != m_sectorData.end())
        ? static_cast<int>(std::distance(m_sectorData.begin() + static_cast<std::ptrdiff_t>(m_sectorDataStart), it))
        : -1;
    
    if (syncPatternPosition == -1) {
        // No sync pattern found
        m_syncHuntNoPatternCount++;

        // Clear the sector data buffer (except the last 11 bytes)
        const size_t bytesBuffered = bufferedBytes();
        if (bytesBuffered > 11) {
            m_discardedBytes += static_cast<uint32_t>(bytesBuffered - 11);
            m_syncHuntDiscardedBytes += static_cast<uint32_t>(bytesBuffered - 11);
            consumeBufferedBytes(bytesBuffered - 11, false);
        }

        // Get more data and try again
        nextState = WaitingForSync;
    } else {
        // Sync pattern found

        // Discard any data before the sync pattern
        m_discardedBytes += syncPatternPosition;
        m_syncHuntDiscardedBytes += static_cast<uint32_t>(syncPatternPosition);
        
        // Keep only data from syncPatternPosition onwards
        consumeBufferedBytes(static_cast<size_t>(syncPatternPosition));

        // Do we really have a valid sector or is this a false positive?

        // Is the sector broken?  Count the total number of error bytes and padding bytes in the sector
        int32_t errorByteCount = 0;
        int32_t paddingByteCount = 0;
        const size_t dataBaseIndex = m_sectorDataStart;
        const size_t errorBaseIndex = m_sectorErrorStart;
        const size_t paddedBaseIndex = m_sectorPaddedStart;
        for (int i = 0; i < 2352; i++) {
            if (static_cast<uint8_t>(m_sectorErrorData[errorBaseIndex + static_cast<size_t>(i)]) == 1) {
                errorByteCount++;
            }
            if (static_cast<uint8_t>(m_sectorPaddedData[paddedBaseIndex + static_cast<size_t>(i)]) == 1) {
                paddingByteCount++;
            }
        }

        if (errorByteCount > 1000 || paddingByteCount > 1000) {
            m_syncHuntFalsePositiveCount++;
            if (errorByteCount == 0 && paddingByteCount == 2352) {
                m_discardedBytes += 2352;
                m_discardedPaddingBytes += 2352;
                m_syncHuntDiscardedBytes += 2352;
                m_syncHuntDiscardedPaddingBytes += 2352;
                m_syncHuntFullPaddingSkipCount++;
                consumeBufferedBytes(2352);
            } else if (syncPatternPosition == 0) {
                m_discardedBytes += 1;
                m_syncHuntDiscardedBytes += 1;
                m_syncHuntZeroPositionFalsePositiveCount++;
                consumeBufferedBytes(1);
            }
            nextState = WaitingForSync;
        } else {
            logSyncHuntSummary("succeeded");
            nextState = InSync;
        }
    }

    return nextState;
}

Data24ToRawSector::State Data24ToRawSector::inSync()
{
    State nextState = InSync;

    // Is there enough data in the buffer to form a sector?
    if (bufferedBytes() < 2352) {
        // Not enough data
        LOG_DEBUG("Data24ToRawSector::inSync(): Not enough data in sectorData to form a sector, waiting for more data");

        // Get more data and try again
        nextState = InSync;
        return nextState;
    } else {
        // Are there any error bytes or padding in the first 12 bytes?
        const size_t dataBaseIndex = m_sectorDataStart;
        const size_t errorBaseIndex = m_sectorErrorStart;
        const size_t paddedBaseIndex = m_sectorPaddedStart;
        auto errorIt = std::find(m_sectorErrorData.begin() + static_cast<std::ptrdiff_t>(errorBaseIndex),
                                 m_sectorErrorData.begin() + static_cast<std::ptrdiff_t>(errorBaseIndex + 12),
                                 1);
        auto paddingIt = std::find(m_sectorPaddedData.begin() + static_cast<std::ptrdiff_t>(paddedBaseIndex),
                                   m_sectorPaddedData.begin() + static_cast<std::ptrdiff_t>(paddedBaseIndex + 12),
                                   1);

        if (errorIt != m_sectorErrorData.begin() + static_cast<std::ptrdiff_t>(errorBaseIndex + 12) ||
            paddingIt != m_sectorPaddedData.begin() + static_cast<std::ptrdiff_t>(paddedBaseIndex + 12)) {
            // Is the sector broken?  Count the total number of error bytes and padding bytes in the sector
            int32_t errorByteCount = 0;
            int32_t paddingByteCount = 0;
            for (int i = 0; i < 2352; i++) {
                if (static_cast<uint8_t>(m_sectorErrorData[errorBaseIndex + static_cast<size_t>(i)]) == 1) {
                    errorByteCount++;
                }
                if (static_cast<uint8_t>(m_sectorPaddedData[paddedBaseIndex + static_cast<size_t>(i)]) == 1) {
                    paddingByteCount++;
                }
            }

            LOG_DEBUG("Data24ToRawSector::inSync(): Sector header corrupt. Sector contains {} error bytes and {} padding bytes",
                         errorByteCount, paddingByteCount);
        }

        // Is there a valid sync pattern at the beginning of the sector data?
        bool syncMatch = (bufferedBytes() >= 12 &&
                 std::equal(m_syncPattern.begin(), m_syncPattern.end(),
                                    m_sectorData.begin() + static_cast<std::ptrdiff_t>(dataBaseIndex)));
        
        if (!syncMatch) {
            // No sync pattern found
            m_missedSyncPatternCount++;
            m_badSyncPatternCount++;

            if (m_missedSyncPatternCount > 4) {
                // Too many missed sync patterns, lost sync
                LOG_DEBUG("Data24ToRawSector::inSync(): Too many missed sync patterns (4 missed), lost sync. Valid sector count: {}", m_validSectorCount);
                nextState = LostSync;
                return nextState;
            } else {
                // Convert first 12 bytes to hex string
                std::string foundPattern;
                char hexBuf[4];
                for (size_t i = 0; i < 12 && i < bufferedBytes(); i++) {
                    snprintf(hexBuf, sizeof(hexBuf), "%02X ", m_sectorData[dataBaseIndex + i]);
                    foundPattern += hexBuf;
                }
                LOG_DEBUG("Data24ToRawSector::inSync(): Sync pattern mismatch - Found: {} Sector count: {} Missed sync patterns: {}",
                             foundPattern, m_validSectorCount, m_missedSyncPatternCount);
            }
        } else {
            // Sync pattern found
            m_goodSyncPatternCount++;

            if (m_missedSyncPatternCount) {
                LOG_DEBUG("Data24ToRawSector::inSync(): Sync pattern found after {} missed sync patterns (resynced)", m_missedSyncPatternCount);
            }

            m_missedSyncPatternCount = 0;
        }

        // Unscramble the sector (bytes 12 to 2351)
        RawSector rawSector;
        std::vector<uint8_t> &rawDataOut = rawSector.mutableData();
        std::vector<uint8_t> &rawErrorDataOut = rawSector.mutableErrorData();
        std::vector<uint8_t> &rawPaddedDataOut = rawSector.mutablePaddedData();

        for (int32_t i = 0; i < 2352; i++) {
            if (i < 12) {
                // Replace the sync pattern (or the EDC will always be wrong)
                rawDataOut[i] = m_syncPattern[i];
                rawErrorDataOut[i] = 0;
                rawPaddedDataOut[i] = 0;
            } else {
                // Only bytes 12 to 2351 are scrambled
                const size_t dataSourceIndex = dataBaseIndex + static_cast<size_t>(i);
                const size_t errorSourceIndex = errorBaseIndex + static_cast<size_t>(i);
                const size_t paddedSourceIndex = paddedBaseIndex + static_cast<size_t>(i);
                rawDataOut[i] = m_sectorData[dataSourceIndex] ^ unscrambleTable[i];
                rawErrorDataOut[i] = m_sectorErrorData[errorSourceIndex];
                rawPaddedDataOut[i] = m_sectorPaddedData[paddedSourceIndex];
            }
        }

        m_outputBuffer.push_back(std::move(rawSector));
        m_validSectorCount++;

        consumeBufferedBytes(2352);
    }

    return nextState;
}

Data24ToRawSector::State Data24ToRawSector::lostSync()
{
    State nextState = WaitingForSync;
    m_missedSyncPatternCount = 0;
    LOG_DEBUG("Data24ToRawSector::lostSync(): Lost sync");
    m_syncLostCount++;
    return nextState;
}

void Data24ToRawSector::showStatistics() const
{
    LOG_INFO("Data24ToRawSector statistics:");
    LOG_INFO("  Valid sectors: {}", m_validSectorCount);
    LOG_INFO("  Discarded bytes: {}", m_discardedBytes);
    LOG_INFO("  Discarded padding bytes: {}", m_discardedPaddingBytes);
    
    LOG_INFO("  Good sync patterns: {}", m_goodSyncPatternCount);
    LOG_INFO("  Bad sync patterns: {}", m_badSyncPatternCount);

    LOG_INFO("  Missed sync patterns: {}", m_missedSyncPatternCount);
    LOG_INFO("  Sync lost count: {}", m_syncLostCount);
}

std::string Data24ToRawSector::statisticsText() const
{
    std::ostringstream output;
    output << "Data24ToRawSector statistics:\n";
    output << "  Valid sectors: " << m_validSectorCount << "\n";
    output << "  Discarded bytes: " << m_discardedBytes << "\n";
    output << "  Discarded padding bytes: " << m_discardedPaddingBytes << "\n";
    output << "  Good sync patterns: " << m_goodSyncPatternCount << "\n";
    output << "  Bad sync patterns: " << m_badSyncPatternCount << "\n";
    output << "  Missed sync patterns: " << m_missedSyncPatternCount << "\n";
    output << "  Sync lost count: " << m_syncLostCount;
    return output.str();
}
